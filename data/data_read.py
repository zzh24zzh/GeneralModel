...
def main():
  ...
  parser.add_option('--cl', dest='cl',
                    default='none')
  parser.add_option('--modal', dest='modal',
                    default='none')
  parser.add_option('--output_dir',dest='output_dir',default='data/')
  (options, args) = parser.parse_args()

  # path to ENCODE black list fle
  options.blacklist_bed='black_list.bed'

  genome_cov_file='%s_%s.bigWig'%(options.cl,options.modal)
  print(options.blacklist_bed,genome_cov_file, options.cl, options.clip)
  seqs_bed_file='input_region_600kb.bed'

  # output file
  seqs_cov_file=os.path.join(options.output_dir,'%s_%s_seq_cov.h5'%(options.cl,options.modal))
